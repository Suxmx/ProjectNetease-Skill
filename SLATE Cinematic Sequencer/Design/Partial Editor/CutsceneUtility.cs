    ...
    public static class CutsceneUtility
    {
        ...
        public static ActionClip PasteClip(CutsceneTrack track, float time)
        {
            if (copyType != null && !string.IsNullOrEmpty(copyJson))
            {
                ...
                if (newAction is IMultiLineClip multi) multi.SetLine(track.GetMaxLine() + 1);
                var nextAction = track.clips.FirstOrDefault(a =>
                    a.startTime > newAction.startTime && a.GetLine() == newAction.GetLine());
