...
namespace Slate
{
    ...
    abstract public class CutsceneTrack : MonoBehaviour, IDirectable
    {
        ...
        [SerializeField, HideInInspector] private bool _isCollapsed = false;
        ...
        public float clipStartTime => clips.Count > 0 ? clips.Min(x => x.startTime) : 0;
        public float clipEndTime => clips.Count > 0 ? clips.Max(x => x.startTime) : 0;
        ...
        virtual public float defaultHeight {
            get
            {
                if (isCollapsed) return Styles.defaultTrackHeight;
                var maxLine  = GetMaxLine();
                if (maxLine <= 1) return Styles.defaultTrackHeight;
                return GetLineHeight(maxLine + 1);
            }
        }

        public int GetMaxLine()
        {
            return Mathf.Max(1, Mathf.Max(clips.OfType<IMultiLineClip>().Select(b => b.GetLine()).ToArray()));
        }
        ...
        public int GetLineHeight(int lineNum)
        {
            ...
            lineNum--;
            return lineNum * Styles.multiLineTrackHeight + (lineNum - 1) * Styles.multiLineTrackMargin;
        }
        ...
        public ActionClip AddAction(System.Type type, float time) {
            ...
            clips.Add(newAction);
            newAction.PostCreate(this);
            if (newAction is IMultiLineClip multi)
            {
                multi.SetLine(GetMaxLine() + 1);
            }
        ...
        {
            var multiLineClips = clips.OfType<IMultiLineClip>().ToList();
            var lines = multiLineClips.Select(c => c.GetLine()).ToList();
            var distinctLines = lines.Distinct().OrderBy(x => x).ToList();

            // 建立旧 line 到新 line 的映射
            var lineMap = new Dictionary<int, int>();
            for (int i = 0; i < distinctLines.Count; i++)
            {
                lineMap[distinctLines[i]] = i + 1;
            }
            foreach (var clip in multiLineClips)
            {
                int oldLine = clip.GetLine();
                if (lineMap.TryGetValue(oldLine, out int newLine))
                {
                    clip.SetLine(newLine);
                }
            }
        }
        ...
        protected void DoDefaultInfoGUI(Event e, Rect trackRect) {
            ...
            float btnVMargin = (trackRect.height - 48) / 2;
            var textInfoRect = Rect.MinMaxRect(iconBGRect.xMax + 2, 0, trackRect.width - BOX_WIDTH - 2, defaultHeight);
            var collapseRect = Rect.MinMaxRect(trackRect.xMax - 24, btnVMargin, trackRect.xMax, trackRect.height - btnVMargin);
            var curveButtonRect = new Rect(trackRect.width - BOX_WIDTH, 0, BOX_WIDTH, defaultHeight);
            var addClipRect = Rect.MinMaxRect(collapseRect.xMin - 24, btnVMargin, collapseRect.xMin, trackRect.height - btnVMargin);
            ...
            var nameString = string.Format("<size=12>{0}</size>", name);
            // var infoString = string.Format("<size=9><color=#909090>{0}</color></size>", info);
            GUI.color = isActive ? Color.white : Color.grey;
            GUI.Label(textInfoRect, $"{nameString}", Styles.leftLabel);
            GUI.color = Color.white;
            if (GUI.Button(addClipRect, UnityEditor.EditorGUIUtility.IconContent("Toolbar Plus More"),Styles.centerLabel))
            {
                ShowAddClipMenu(0);
            }
            var collapseTxt = isCollapsed ? "►" : "▼";
            if (GUI.Button(collapseRect, collapseTxt, Styles.centerLabel))
            {
        ...
        void DoTrackContextMenu(Event e, Rect clipsPosRect, float cursorTime)
        {
            ...
            {
                if (TryShowRightClickMenu(cursorTime))
                {
                    e.Use();   
                }
            }
        ...
        protected List<EditorTools.TypeMetaInfo> GetAttachables()
        {
            ...
            return attachableTypeInfos;
        }

        protected void ShowAddClipMenu(float time)
        {
            var attachableTypeInfos = GetAttachables();
            if (attachableTypeInfos.Count > 0)
            {
                ...
                foreach (var _info in attachableTypeInfos)
                {
                    ...
                    var tName = info.name;
                    menu.AddItem(new GUIContent(category + tName), false, () => { AddAction(info.type, time); });
                }
                menu.ShowAsContext();
            }
        }
        protected bool TryShowRightClickMenu(float time)
        {
            ...
            if (attachableTypeInfos.Count > 0)
            {
                ...
                if (copyType != null && attachableTypeInfos.Select(i => i.type).Contains(copyType))
                {
                    ...
                    menu.AddItem(new GUIContent(string.Format("Paste Clip ({0})", copyType.Name)), false,
                        () => { CutsceneUtility.PasteClip(this, time); });
                }
        ...
        public void AdjustClipsAfterMultiOperation()
        {
            // 按行分组
            var lineGroups = clips.GroupBy(c => c.GetLine());
            foreach (var group in lineGroups)
            {
                // 按startTime排序
                var sortedClips = group.OrderBy(c => c.startTime).ToList();
                ActionClip prev = null;
                foreach (var clip in sortedClips)
                {
                    float minStart = 0;
                    if (prev != null)
                    {
                        minStart = prev.endTime;
                        // 如果可以CrossBlend，允许交叠
                        if (clip.CanCrossBlend(prev))
                        {
                            prev = clip;
                            continue;
                        }
