...
namespace Hoshino
{
    ...
    public class SkillEditor : EditorWindow
    {
        ...
        private const float TOP_MARGIN = 22; //top margin AFTER the toolbar
        ...
        private const float BOTTOM_MARGIN = 40;
        ...
        [System.NonSerialized] private float timeInfoStep;
...
#pragma warning restore
        ...
        void DoKeyboardShortcuts() {
            ...
            if ( e.type == EventType.KeyDown && GUIUtility.keyboardControl == 0 && !e.control && !e.shift ) {
                ...
                if ( e.keyCode == KeyCode.Space ) {
                    if ( editorPlaybackState != EditorPlaybackState.Stoped ) { Pause(); } else { Play(); }
                    e.Use();
                }
                    ...
                    if ( multiSelection != null ) {
                        ...
                        if ( clip != null ) {
                            SafeDoAction(() =>
                            {
                                ...
                                (clip.parent as CutsceneTrack).RemoveEmptyLines();
                                InitClipWrappers();
                            });
                            e.Use();
                        }
        ...
        void ShowToolbar() {
            ...
            if ( GUILayout.Button($"[{cutscene.name}]", EditorStyles.toolbarDropDown, GUILayout.Width(100)) ) {
            ...
            if ( GUILayout.Button("Snap: " + Prefs.snapInterval.ToString(), EditorStyles.toolbarDropDown, GUILayout.Width(130)) ) {
                var menu = new GenericMenu();
            ...
            if (isStoped)
            {
                ...
                if (GUILayout.Button(Styles.playIcon, EditorStyles.toolbarButton, GUILayout.Width(25)))
                {
                    Play();
                    Event.current.Use();
                }
            ...
            {
                if (GUILayout.Button(Styles.pauseIcon, EditorStyles.toolbarButton, GUILayout.Width(25)))
                {
                    Pause();
                    Event.current.Use();
                }
            ...
            if (GUILayout.Button(Styles.stopIcon,EditorStyles.toolbarButton, GUILayout.Width(25)))
            {
                if (editorPlaybackState == EditorPlaybackState.PlayingForwards) Pause();
                Stop(false);
                Event.current.Use();
            }
            ...
            if (GUILayout.Button(Styles.stepIcon, EditorStyles.toolbarButton, GUILayout.Width(25)))
            {
                if(editorPlaybackState == EditorPlaybackState.PlayingForwards)Pause();
                cutscene.currentTime += 1.0f / Prefs.frameRate;
                Event.current.Use();
            }
        ...
        void ShowTimeInfo(Rect topMiddleRect) {
            ...
            timeInfoStep = doFrames ? ( 1f / Prefs.frameRate ) : lowMod;
            ...
            GUI.BeginGroup(topMiddleRect);
            {
                ...
                //the step interval
                if ( centerRect.width / ( viewTime / timeInfoStep ) > 4 ) {
                    for ( var i = timeInfoStart; i <= timeInfoEnd; i += timeInfoStep ) {
                        var posX = TimeToPos(i);
                        var frameRect = Rect.MinMaxRect(posX - 0.5f, TOP_MARGIN - 2, posX + 0.5f, TOP_MARGIN);
                ...
                for ( var i = timeInfoStart; i <= timeInfoEnd; i += timeInfoInterval ) {
                    ...
                    GUI.color = isProSkin ? Color.white : Color.black;
                    var markRect = Rect.MinMaxRect(posX - 1, TOP_MARGIN - 5, posX + 1, TOP_MARGIN);
                    ...
                    if (viewTimeMin == 0 && Mathf.Approximately(i, timeInfoStart))
                    {
                        var delta = -stampRect.xMin;
                        stampRect.xMin += delta;
                        stampRect.xMax += delta;
                    }
        ...
        void DrawRedGuideLineBtn()
        {
            if (cutscene.currentTime < viewTimeMin || cutscene.currentTime > viewTimeMax) return;
            var xPos = TimeToPos(cutscene.currentTime);
            var text = cutscene.currentTime.ToString(format: "0.00");
            var size = GUI.skin.GetStyle("label").CalcSize(new GUIContent(text));
            var guideRect = new Rect(xPos + centerRect.x - 1, guideSlideRect.yMin - 10,
                size.x, 15);
        ...
        void ShowGroupsAndTracksList(Rect leftRect) {
            ...
            var listTopRect = Rect.MinMaxRect(leftRect.x, leftRect.y, leftRect.xMax, leftRect.y + TOP_MARGIN);
            GUI.Label(listTopRect,"测试占位文本",Styles.leftLabel);
        ...
        void ShowListTracks(Event e, CutsceneGroup group, ref float nextYPos) {
            ...
            for ( int t = 0; t < group.tracks.Count; t++ ) {
                ...
                var trackRect = new Rect(5, yPos, leftRect.width - TRACK_RIGHT_MARGIN - 5, track.finalHeight);
                nextYPos += track.finalHeight + TRACK_MARGINS;

                //GRAPHICS
                GUI.color = Slate.ColorUtility.Grey(isProSkin ? ( track.isActive ? 0.25f : 0.2f ) : ( track.isActive ? 0.9f : 0.8f ));
        ...
        void ShowTimeLines(Rect centerRect) {
            ...
            if ( centerRect.width / ( viewTime / timeInfoStep ) > 4 ) {
                for ( var i = timeInfoStart; i <= timeInfoEnd; i += timeInfoStep ) {
                    DrawGuideLine(i, Color.black.WithAlpha(0.15f));
                }
            }
            ...
            for ( int g = 0; g < cutscene.groups.Count; g++ ) {
                ...
                for ( int t = 0; t < group.tracks.Count; t++ ) {
                    ...
                    {
                        var startClipX = TimeToPos(track.clipStartTime);
                        var endClipX = TimeToPos(track.clipEndTime);
                        var clipRect = Rect.MinMaxRect(startClipX,yPos, endClipX,yPos + track.finalHeight);
                        var color = track.color;
                        GUI.color = color;
                        GUI.DrawTexture(clipRect.ExpandBy(-1), Styles.whiteTexture);
                        GUI.color = Color.black;
                        var label =  track.GetType().Name.Replace("Track","");
                        var size = Styles.leftLabel.CalcSize(new GUIContent(label));
                        if (size.x > clipRect.width)
                        {
                            label = label.Substring(0, Mathf.FloorToInt(label.Length * (clipRect.width / size.x))) + "...";
                        }
                        GUI.Label(clipRect, label, Styles.leftLabel);
                        GUI.color = Color.white;
                    }
                    else
                    {
                        var trackMaxLine = track.GetMaxLine();
                        //ACTION CLIPS
                        for (int a = 0; a < track.clips.Count; a++)
                        {
                            ...
                            var nextClip = track.clips.FirstOrDefault(x =>
                                x.GetLine() == action.GetLine() && x.startTime > action.startTime);
                            var previousClip = track.clips.LastOrDefault(x =>
                                x.GetLine() == action.GetLine() && x.startTime < action.startTime);
                            ...
                            //modify it
                            clipRect.y = yPos;
                            clipRect.width = Mathf.Max(action.length / viewTime * centerRect.width, 12);
                            if (trackMaxLine == 1)
                            {
                            ...
                            {
                                clipRect.height = Styles.multiLineTrackHeight;
                                clipRect.y += track.GetLineHeight(action.GetLine());
                            }
                                ...
                                interactingClip.isDragging)
                            {
                                ...
                                if (multiSelection == null || multiSelection.Count < 1)
                                {
                                    var preCursorClip = track.clips.LastOrDefault(x =>
                                        x != action && x.startTime < cursorTime && x.GetLine() == action.GetLine());
                                    var postCursorClip = track.clips.FirstOrDefault(x =>
                                        x != action && x.endTime > cursorTime && x.GetLine() == action.GetLine());
                                    ...
                                    if (Prefs.magnetSnapping && !e.control)
                                    {
                                        ...
                                        if (snapStart != null && snapEnd != null)
                                        {
                                            ...
                                            xTime = distEnd < distStart
                                                ? snapEnd.Value - action.length
                                                : snapStart.Value;
                                        }
                                        else
                                        {
                            ...
                            if (isSelected)
                            {
                                var selRect = clipRect.ExpandBy(1);
            ...
            if ( multiSelectStartPos != null ) {
                ...
                if ( ( start - e.mousePosition ).magnitude > 10 ) {
                    ...
                    foreach ( var wrapper in clipWrappers.Values.Where(b => r.Overlaps(b.rect) && !b.action.isLocked) ) {
                        ...
                        GUI.DrawTexture(wrapper.rect, Styles.whiteTexture);
                        GUI.color = Color.white;
                    }
                }
            }
        ...
        private void DrawMinMaxSlider()
        {
            ...
            var sliderRect = new Rect(centerRect.xMin + 5, centerRect.yMax, centerRect.width - 10, 18);
        ...
        class ActionClipWrapper
        {
            ...
            private SkillEditor editor {
                get { return SkillEditor.current; }
            }
            ...
            public void ResetInteraction() {
                ...
                isScalingEnd = false;
                (action.parent as CutsceneTrack)?.AdjustClipsAfterMultiOperation();
            }
                        ...
                        editor.SafeDoAction(() =>
                           {
                               foreach ( var act in multiSelection.Select(b => b.action).ToArray() ) {
                                   ( act.parent as CutsceneTrack ).DeleteAction(act);
                                   ( act.parent as CutsceneTrack ).RemoveEmptyLines();
                               }
